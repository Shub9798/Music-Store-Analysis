...
set sql_mode="";
load data infile "album.csv" into table album
...
select c.customer_id,concat( c.first_name,' ', c.last_name) as `full name`,sum(i.total ) as money_spent from customer c inner join invoice i on c.customer_id= i.customer_id
...
select i.billing_country, (il.unit_price*il.quantity) as purchase_amount, g.name 
...
select name,concat(round( milliseconds/1000,2)," K") as `length millisec` from track
where milliseconds > (select avg(milliseconds) as avg_length from track)
order by milliseconds desc;

# Advance
#1. Find how much amount spent by each customer on artists? Write a query to return customer name, artist name and total spent
select concat(c.first_name," ",c.last_name) as customer_name,a.name as artist_name,sum(il.unit_price*il.quantity) as total_spent from
customer c inner join invoice i using(customer_id) 
inner join invoice_line il using(invoice_id) 
inner join track t using(track_id) inner join album al using(album_id) inner join artist a using(artist_id)
...
/*3. Write a query that determines the customer that has spent the most on music for each country.
 ...
 select  customer_name,  country, amount_spent
from ( select concat(c.first_name," ",c.last_name) as customer_name,c.country as country,
